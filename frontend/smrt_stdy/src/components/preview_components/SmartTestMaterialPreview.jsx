...
const SmartTestMaterialPreview = ({ isOpen, onClose, initialData, sourceData }) => {
...
const sendGenDataToDB = () => {
  ...
  axios
    .post("http://localhost:8000/save_smart_test", {
      type: "SmartTest",
      author_id: "",
      created_at: "",
      topic: modalData.topic,
      num_questions: modalData.num_questions,
      source: sourceData,
...
<div className="font-bold text-2xl text-gray-800 mb-6">
    ...
    <p className="text-gray-600 text-lg">
           Питання: {modalData.num_questions}
          </p>
      </div>
        {/* Теги */}
...
<div className="carousel h-20 w-full mb-6">
  {modalData.questions && modalData.questions.length > 0 ? (
    modalData.questions.map((test, index) => (
      <div
        key={index}
        id={`slide${index}`}
            className="carousel-item relative w-full flex flex-col items-center justify-center text-center">
        {/* Питання */}
        <p className="text-lg text-gray-800 w-[80%] mb-4">{test.question}</p>

        {/* Кнопки навігації */}
        <div className="absolute left-5 right-5 top-1/2 flex -translate-y-1/2 transform justify-between">
          <a
            href={`#slide${index - 1 >= 0 ? index - 1 : modalData.questions.length - 1}`}
                className="btn rounded-xl text-gray-700 bg-gray-50 hover:bg-gray-100">

            ❮
          </a>
          <a
            href={`#slide${index + 1 < modalData.questions.length ? index + 1 : 0}`}
                className="btn rounded-xl text-gray-700 bg-gray-50 hover:bg-gray-100">
    ...
    <p className="text-center">Немає карток</p>
  )}
</div>
...
export default SmartTestMaterialPreview;
