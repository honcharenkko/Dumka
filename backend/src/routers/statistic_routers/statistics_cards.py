...
from src.mongo_db.data_structures import StatisticSmartCards
...
router = APIRouter(
    prefix="/statistic_cards",
    tags= ["Statistic"]
)

@router.post("/insert_cards_stat", response_model=StatisticSmartCards)
async def create_cards_stat(stat: StatisticSmartCards, request: Request):
    ...
    try:
        ...
        await statistic_collection.insert_one(stat.dict(exclude_unset=True))  # ✅ clean insert
...
@router.patch("/update_cards_stat/{item_id}", response_model=StatisticSmartCards)
async def update_cards_stat(item_id: str, stat: StatisticSmartCards, request: Request):
    token = request.cookies.get(auth_config.JWT_ACCESS_COOKIE_NAME)
    if not token:
        raise HTTPException(status_code=401, detail="Token missing")
...
@router.get("/get_cards_stat/{item_id}", response_model=StatisticSmartCards)
async def get_cards_stat(item_id: str, request: Request):
    token = request.cookies.get(auth_config.JWT_ACCESS_COOKIE_NAME)
    if not token:
        raise HTTPException(status_code=401, detail="Token missing")
    ...
    return StatisticSmartCards(**stat)


@router.delete("/delete_cards_stat/{item_id}")
